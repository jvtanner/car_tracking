...
class ExactInference(object):
    ...
    def observe(self, agentX, agentY, observedDist):
        ...
        for row in range(self.belief.getNumRows()):
            for col in range(self.belief.getNumCols()):
                ...
                Y = util.rowToY(row)
                # Mean
                your_car = math.sqrt((agentX - X) ** 2 + (agentY - Y) ** 2)
                # Likelihood
                likelihood = util.pdf(your_car, Const.SONAR_STD, observedDist)
                # Posterior
    ...
    def elapseTime(self):
        ...
        for row in range(self.belief.getNumRows()):
            for col in range(self.belief.getNumCols()):
                obs[(row, col)] = self.belief.getProb(row, col)
                self.belief.setProb(row, col, 0)
        for next in self.transProb:
            trans = self.transProb[next] * obs[next[0]]
            self.belief.addProb(next[1][0], next[1][1], trans)
        self.belief.normalize()
        # END_YOUR_CODE
...
class ParticleFilter(object):
    ...
    # Function: Init
    # --------------
    # Constructor that initializes a ParticleFilter object which has
    # (numRows x numCols) number of tiles.
    def __init__(self, numRows, numCols):
        self.belief = util.Belief(numRows, numCols)
    ...
    def observe(self, agentX, agentY, observedDist):
        ...
        parDict = collections.defaultdict(float)
        for particle in self.particles:
            row = particle[0]
            col = particle[1]
            Y = util.rowToY(row)
            X = util.colToX(col)
            prior = self.particles[particle]
            your_car = math.sqrt((agentX - X) ** 2 + (agentY - Y) ** 2)
            likelihood = util.pdf(your_car, Const.SONAR_STD, observedDist)
            post = prior * likelihood
            parDict[particle] = post
        self.particles = collections.defaultdict(int)
        for _ in range(self.NUM_PARTICLES):
            updatedPart = util.weightedRandomChoice(parDict)
            self.particles[updatedPart] += 1
        # END_YOUR_CODE
    ...
    def elapseTime(self):
        # BEGIN_YOUR_CODE (our solution is 6 lines of code, but don't worry if you deviate from this)
        parDict = collections.defaultdict(int)
        for tile in self.particles:
            for _ in range(self.particles[tile]):
                nextTile = util.weightedRandomChoice(self.transProbDict[tile])
                parDict[nextTile] += 1
        self.particles = parDict
        # END_YOUR_CODE
